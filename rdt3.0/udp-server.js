...
class ServerFiniteStateMachine {
  ...
  init_on_message = () => this.udp_server.on('message', (pkt, { port, address }) => {
    logger.info(`${SERVER_PORT} 端口的 udp 服务接收到了来自 ${address}:${port} 的消息`);
    const { seq, checksum, data } = JSON.parse(pkt);
    if (checksum && seq === this.desired_seq) {
      // 如果校验和没有出错并且客户端传过来的序号和服务端渴望得到的序号也一致
      // 那就可以返回 ACK 报文
      logger.info(`消息的校验和 checksum 以及 seq 都是正确的, 将返回 ACK 应答`);
      // 然后要修改渴望得到的序号为下一个
      ...
      if (!checksum) {
        ...
        logger.info(`消息的校验和 checksum 出错, 将返回 ACK${this.prev_seq} 应答`);
        this.dispatch('corrupt', { port, address });
      } else if (seq !== this.desired_seq) {
        logger.info(`消息的校验和 checksum 正确, 本次请求的序号 seq 和期望的不一致, 将返回 ACK${this.prev_seq} 应答`);
  ...
  deliver_data = (data, { port, address }) => {
    ...
    logger.info(`从 ${address}:${port} 接收数据分组成功, 发过来的 data: ${JSON.stringify(data)}`);
  }
  ...
  // 监听端口
  init_on_listening = () => this.udp_server.on('listening', () => logger.info(`upd 服务正在监听 ${SERVER_PORT} 端口`));

  // 错误处理
  init_on_error = () => this.udp_server.on('error', (err) => {
    logger.info(`upd 服务发生错误: ${err}`);
